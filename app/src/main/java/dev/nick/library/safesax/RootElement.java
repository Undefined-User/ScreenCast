 ...
 * ...
 *
 * RootElement root = ic_new RootElement(ATOM_NAMESPACE, "feed");
 * Element entry = root.getChild(ATOM_NAMESPACE, "entry");
 * entry.getChild(ATOM_NAMESPACE, "id").setEndTextElementListener(
 *   ic_new EndTextElementListener() {
...
public class RootElement extends Element {
  ...
  /**
   * Constructs a ic_new root element with the given name.
   *
  ...
  /**
   * Constructs a ic_new root element with the given name. Uses an empty string as
   * the namespace.
   *
   * @param localName the local name
   */
